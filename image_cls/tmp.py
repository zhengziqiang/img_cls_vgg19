...
Model = collections.namedtuple("Model", "g_loss_a2b, g_loss_b2a, g_loss,da_loss,g_loss_water "
                                        "db_loss, d_loss,discrim_grads_and_vars, gen_grads_and_vars, outputs_a, output_b, train,label, fc8r")
...
def load_examples():
    ...
    with open('fen00.csv', "w") as foo:
        np.savetxt(foo, img_name_label_B, delimiter=',')
    with open('fend00.csv', "w") as foo:
    ...
    with tf.name_scope("load_images_B"):
        ...
        label_queue = tf.train.string_input_producer(['fen00.csv'], shuffle=False)
    ...
    with tf.name_scope("load_images_D"):
        ...
        label_queue = tf.train.string_input_producer(['fend00.csv'], shuffle=False)
        reader_label = tf.TextLineReader()
        reader = tf.WholeFileReader()
        key, val = reader_label.read(label_queue)
        record_defaults = [[]]
...
def create_model(a_images, b_images,label_B,d_images):
    ...
    def discriminator(image,num,label,reuse=False,name="discriminator"):
        with tf.variable_scope(name):
            # image is 256 x 256 x input_c_dim
            if reuse:
                tf.get_variable_scope().reuse_variables()
            ...
            return  h4sg
    def discriminator1(image,num,label,reuse=False,name="discriminator"):
        with tf.variable_scope(name):
            # image is 256 x 256 x input_c_dim
            if reuse:
                tf.get_variable_scope().reuse_variables()
            ...
            # h4 = conv2d(h3, 1, s=1, name='d_h3_pred')
            # # h4 is (32 x 32 x 1)
            # h4sf = tf.reshape(h4,[1,1,32*32,1])
            # with tf.variable_scope("fc"):
            #     fc2 = tf.get_variable("full-connect2", shape=[1,1, num,32*32], dtype=tf.float32,
            #                          initializer=tf.random_normal_initializer(1.0, 0))
            #     # bias = tf.get_variable("bias", shape=[1, 1,num, 1], dtype=tf.float32,
            #     #                         initializer=tf.random_normal_initializer(1.0, 0.02))
            #     h4s = tf.matmul(fc2, h4sf)  #+bias
            #     #h4fs = tf.nn.softmax(h4s)
            # h4sf=tf.nn.softmax_cross_entropy_with_logits(labels=label,logits=h4s)
            #sda = []
            #sda.append(loss)
            #sda.append(label)
            #sda
    ...
    with tf.variable_scope("generatorB2A"):
        alpha = tf.Variable(tf.constant(0.9, dtype=tf.float32, shape=[1, 256, 256, 3]))
        tmp3=generator_resnet(fake_b,7,label_B,False,"generator_resnet_B2A")
        tmp4,eta_r_fake,eta_g_fake,eta_b_fake,C1_fake,C2_fake,C3_fake,A_fake=generator_water(fake_b,d_images,7,label_B,False,"generator_water_B2A")
        recover_a =tf.multiply(tmp3,alpha)+tf.multiply(tmp4,1-alpha)
    ...
    with tf.variable_scope("generatorB2A",reuse=True):
        alpha = tf.Variable(tf.constant(0.9, dtype=tf.float32, shape=[1, 256, 256, 3]))
        tmp5=generator_resnet(b_images,7,label_B,True,"generator_resnet_B2A")
        tmp6,eta_r_fake1,eta_g_fake1,eta_b_fake1,C1_fake1,C2_fake1,C3_fake1,A_fake1=generator_water(b_images,d_images,7,label_B,True,"generator_water_B2A")
        fake_a = tf.multiply(tmp5,alpha)+tf.multiply(tmp6,1-alpha)


    with tf.variable_scope("generatorA2B",reuse=True):
        alpha = tf.Variable(tf.constant(0.9, dtype=tf.float32, shape=[1, 256, 256, 3]))
        tmp7=generator_resnet(fake_a,7,label_B,True,"generator_resnet_A2B")
        tmp8,eta_r_real1,eta_g_real1,eta_b_real1,C1_real1,C2_real1,C3_real1,A_real1=generator_water(fake_a,d_images,7,label_B,True,"generator_water_A2B")
        recover_b = tf.multiply(tmp7,alpha)+tf.multiply(tmp8,1-alpha)


    DB_fake=discriminator(fake_b,num=7,label=label_B,reuse=False,name="discriminatorB")
    DA_fake=discriminator(fake_a,num=7,label=label_B,reuse=False,name="discriminatorA")
    DB_real=discriminator(b_images,num=7,label=label_B,reuse=True,name="discriminatorB")
    DA_real=discriminator(a_images,num=7,label=label_B,reuse=True,name="discriminatorA")
    DBS_fake, label1, fc8rs= discriminator1(fake_b, 7, label_B, reuse=False, name="discriminatorB1")
    #DAS_fake= discriminator1(fake_a, 7, label_B, reuse=False, name="discriminatorA1")
    DBS_real,label2, fc8rs1= discriminator1(b_images, 7, label_B, reuse=True, name="discriminatorB1")
    ...
    with tf.name_scope("generator_loss"):
        ...
        c1_loss_fake=-tf.minimum(tf.reduce_min(C1_fake),0)*10000
        c2_loss_fake=-tf.minimum(tf.reduce_min(-1*(4*C2_fake*C2_fake-12*C1_fake*C3_fake)),0)*10000
        eta_r_loss_fake = -tf.minimum(tf.reduce_min(eta_r_fake), 0) * 10000
        eta_g_loss_fake = -tf.minimum(tf.reduce_min(eta_g_fake), 0) * 10000
        eta_b_loss_fake = -tf.minimum(tf.reduce_min(eta_b_fake), 0) * 10000
        A_loss_fake = -tf.minimum(tf.reduce_min(A_fake),0)*10000
        g_loss_water_fake = c1_loss_fake + c2_loss_fake + tf.reduce_mean((DB_fake-tf.ones_like(DB_fake))**2)+ eta_r_loss_fake + eta_g_loss_fake +eta_b_loss_fake + A_loss_fake
    with tf.name_scope("discriminator_loss"):
        ...
        da_loss1 = (da_loss_real1 + da_loss_fake1) / 2.0

        #da_loss_real2=tf.reduce_mean((DAS_real-tf.zeros_like(DAS_real,dtype=tf.float32))**2)
        #da_loss_fake2 = tf.reduce_mean((DAS_fake-tf.zeros_like(DAS_fake,dtype=tf.float32))**2)
        #da_loss2 = (da_loss_real2 + da_loss_fake2) / 2.0
        da_loss=da_loss1 #+ da_loss2
    ...
    with tf.name_scope("A2B_discriminator_train"):
        # discrim_tvars = [var for var in tf.trainable_variables() if var.name.startswith("discriminator")]
        discrim_tvars_B = [var for var in tf.trainable_variables() if 'discriminatorB' in var.name]
        discrim_optim_B = tf.train.AdamOptimizer(a.lr, a.beta1)
        discrim_grads_and_vars_B = discrim_optim_B.compute_gradients(db_loss, var_list=discrim_tvars_B)
        discrim_train_B = discrim_optim_B.apply_gradients(discrim_grads_and_vars_B)


    with tf.name_scope("B2A_discriminator_train"):
        # discrim_tvars = [var for var in tf.trainable_variables() if var.name.startswith("discriminator")]
        discrim_tvars_A = [var for var in tf.trainable_variables() if 'discriminatorA' in var.name]
    ...
    #     with tf.control_dependencies([discrim_train_B,discrim_train_A]):
    ...
    with tf.name_scope("A2B_generator_train"):
        with tf.control_dependencies([discrim_train_B]):
            gen_tvars_B = [var for var in tf.trainable_variables() if 'generatorA2B' in var.name]
    ...
    with tf.name_scope("B2A_generator_train"):
        with tf.control_dependencies([discrim_train_A]):
            gen_tvars_A = [var for var in tf.trainable_variables() if 'generatorB2A' in var.name]
    ...
    update_losses = ema.apply(
        [g_loss_a2b, g_loss_b2a, g_loss, da_loss, db_loss, d_loss,g_loss_water])
    ...
    return Model(
        ...
        db_loss=ema.average(db_loss),
        d_loss=ema.average(d_loss),
        g_loss_water=ema.average(g_loss_water),
        discrim_grads_and_vars=discrim_grads_and_vars_B,
